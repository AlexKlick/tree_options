...
| `m3-final-gate3-run.sh` / `m3-gate3-inner.sh` | 2026-08-21 | The gate3 entrypoint + inner script (verbatim `m0_gate.sh`, only the mutation output names changed to `artifacts/m3-gate3-mutations.{json,md}`), retained for the record; gate3 re-ran the full gate at the docs-only final head on main after the PR #6 merge (0e010f7). Its log lands here with the follow-up docs commit. |
