...
def test_content_identity_changes_when_content_fields_change():
    # Checkout-bound code/packet hashes are blanked; content-bearing inputs
    # still move both identities.
    base = _identity()
...
def test_packet_hash_moves_run_id_but_not_same_content_identity():
    moved = _identity(verified_packet_sha256="3" * 64)
    assert sealed_run_id(_identity()) != sealed_run_id(moved)
    assert content_identity(_identity()) == content_identity(moved)
